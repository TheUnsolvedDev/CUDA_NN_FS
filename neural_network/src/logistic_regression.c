 ...
 * @Last Modified time: 2023-12-28 19:57:34
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
void logistic_regression(float **dataset, int num_rows, int num_cols, int batch_size, int epochs, float alpha)
{
    ...
    for (int epoch = 0; epoch < epochs; epoch++)
    {
        ...
        for (int batch = 0; batch < list_size; batch++)
        {
            ...
            accuracy_value += binary_accuracy(y_batch, y_pred);
        ...
        alpha *= 0.9 * (epoch % 100 == 0) + (epoch % 100 != 0);
